...
export default {
  ...
  emits: ['update:visible', 'permission-granted'],
  setup(props, { emit }) {
    const formRef = ref()
    const form = reactive({
      itemName: '',
    ...
    const rules = {
      permissionRole: [{ required: true, message: 'Please select permission role', trigger: 'change' }],
      expirationDate: [{ required: true, message: 'Please select expiration date', trigger: 'change' }]
    }

    watch(() => props.permissionForm, (newForm) => {
      if (newForm) Object.assign(form, newForm)
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        emit('permission-granted', permission)
        handleClose()
      } catch (error) {
    ...
    const resetForm = () => {
      Object.assign(form, { itemName: '', itemPath: '', itemType: '', permissionRole: '', expirationDate: '', inheritToSubItems: false, remark: '' })
