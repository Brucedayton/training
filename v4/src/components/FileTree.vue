...
export default {
  ...
  emits: ['apply-permission', 'selection-change'],
  setup(props, { emit }) {
    const treeRef = ref()
    const defaultProps = {
      children: 'children',
  ...
  },
  {
    ...
    const handleCheckChange = () => {
      ...
      // includeHalfChecked=false, leafOnly=false to return all checked nodes
      const checked = treeRef.value.getCheckedNodes(false, false)
      const items = checked.filter(n => n.type === 'file' || n.type === 'folder')
