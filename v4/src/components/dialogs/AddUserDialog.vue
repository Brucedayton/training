...
export default {
  ...
  emits: ['update:visible', 'user-added'],
  setup(props, { emit }) {
    const formRef = ref()
    const form = reactive({ name: '', email: '', role: '', status: 'active' })
    const rules = {
    ...
    const resetForm = () => {
      Object.assign(form, { name: '', email: '', role: '', status: 'active' })
      if (formRef.value) {
    ...
    return { formRef, form, rules, handleClose, handleSubmit, getRoleText }
  }
}
</script>
